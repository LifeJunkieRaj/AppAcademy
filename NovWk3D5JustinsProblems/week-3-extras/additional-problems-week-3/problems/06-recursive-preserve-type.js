...
function recursivePreserveType(array) {
    let preserveType = (dataType) => {
        let performOperation= ( index= 0, result= [])=>{
            if (index>=array.length){
                return result;// we got the value needed
            }
            if (typeof array[index]=== dataType){
                result.push(array[index]);
            }
            return performOperation(index+1, result ); // recurssion
        }
...
const preserveFunc2 = recursivePreserveType([2, undefined, 'world', {
    color: 'red'
}, true, 3, [4, 5], 'hello', false]);
console.log(preserveFunc2('number')); // prints [2, 3]
console.log(preserveFunc2('object')); // prints [ { color: 'red' }, [4, 5] ]
console.log(preserveFunc2('boolean')); // prints [ true, false ]
